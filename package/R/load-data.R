...
loadTable <- function(site, table) {
  ...
  df <- read_delim(file = filename, delim = delim, col_names = columns, col_select = all_of(select), col_types = types, skip = 1)
  if (isCents && table == 'catalog') df <- df %>% mutate(price = price / 100)
  df
}

getFinalTable <- function(orders, details, catalog) orders %>%
...
loadSite <- function(site) {
  ...
  # Build result table
  sold <- getFinalTable(orders, details, catalog)
